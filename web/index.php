    ...
    function getTracks($limit, $tokenSpotify, $mysqli, $result)
    {
        ...
        error_log("https://api.spotify.com/v1/me/top/tracks?time_range=long_term&limit=".$limit."&offset=0&access_token=".$tokenSpotify);
            ...
            if($resultString == "" || $resultString == " ") {
                ...
                $res_active = $res->fetch_assoc();
                $output = json_decode(shell_exec("curl -H \"Authorization: Basic " . AUTHORISATION_BASE_64_SPOTIFY . "\" -d grant_type=refresh_token -d refresh_token=" . $res_active["refreshTokenSpotify"] . " -d redirect_uri=https://music-statuc-by-kos.herokuapp.com/spotify https://accounts.spotify.com/api/token --ssl-no-revoke"));
